...
# Delete Bit.ly story statistics from database and GridFS
#
# Params:
# * $db - database object
# * $stories_id - story ID
#
# Returns true on success; die()s on error
sub delete_story_stats($$)
{
    my ( $db, $stories_id ) = @_;
    ...
    unless ( story_stats_are_fetched( $db, $stories_id ) )
    {
        say STDERR "Story $stories_id is not processed with Bit.ly.";
        return 1;
    }

    # Transaction will be rolled back if GridFS fails
    $db->begin;

    $db->query(
        <<EOF,
        DELETE FROM bitly_story_daily_clicks
        WHERE stories_id = ?
EOF
        $stories_id
    );
...
EOF
    ...
    eval { $_gridfs_store->remove_content( $db, $stories_id ); };
    if ( $@ )
    {
        $db->rollback;
        die "GridFS died while removing Bit.ly stats for story $stories_id: $@\n";
    }
...
# Delete Bit.ly controversy stories' statistics from database and GridFS
#
# Params:
# * $db - database object
# * $controversies_id - controversy ID
#
# Returns true on success; die()s on error
sub delete_controversy_stats($$)
{
...
EOF
        ...
        foreach my $story ( @{ $stories } )
        {
            ...
            delete_story_stats( $db, $stories_id );
        }
